...
Whenever an issue is created in Vantik and labeled as `bug` then create an action which call OpenAI to search for possible solution and put the solution as a comment of that issue.
