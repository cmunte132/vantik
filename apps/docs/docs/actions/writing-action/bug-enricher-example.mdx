...
Whenever an issue is created in Vantik and labeled as `bug` then create an action which asks the configured AI model to search for a possible solution and puts that solution in a comment on the issue.
