...
Below is an example to help you understand how you can create a action on any Vantik event.

## Example Action

#### Bug Enricher

Whenever an issue is created in Vantik and labeled as `bug` then create an action that asks the configured AI model to search for a possible solution and posts that solution in a comment on the issue.
...
2. Now run our cli command, this will login the cli into Vantik.
...
4. Let's edit the config.json as below to tell Vantik to trigger the function when issue is created.
...
You can check full prompt [here](https://github.com/vantikhq/vantik/blob/main/actions/bug-enricher/utils.ts).
...
You can find the whole code for this example action
  [here](https://github.com/vantikhq/vantik/tree/main/actions/bug-enricher)
:::
