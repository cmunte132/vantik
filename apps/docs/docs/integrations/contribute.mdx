...
Welcome to the Vantik integration development guide! This guide will walk you through the steps to create a new integration for Vantik.

## Prerequisites

<details>
<summary>Before you begin, ensure you have the following installed</summary>
...
1. Navigate to the `/app/integrations` folder in your Vantik project:
   ```bash
   cd /app/integrations
   ```
...
Vantik integrations are built on top of a unified event handler that processes different event types within a single function. The core of your integration lies in how you handle these event types.
...
- **IntegrationCreate:** This event is triggered when a user provides authentication details within Vantik. Your handler should process the user's input and handle the creation of the integration, such as saving settings data and establishing necessary connections.
...
Questions, comments, or concerns? Let us know in our github: https://github.com/vantikhq/vantik/issues