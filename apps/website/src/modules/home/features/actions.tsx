import { cn } from '@vantikhq/ui/lib/utils';
import Image from 'next/image';

export function Actions() {
  return (
    <div className="flex flex-col mt-16 gap-6 text-md">
      <h4
        className={cn(
          'text-balance max-w-2xl text-xl font-medium text-foreground',
        )}
      >
        Write, deploy and automate anything
      </h4>

      <div className="relative grid border-none justify-center grid-cols-1 sm:grid-cols-12 divide-x-0 grid-rows-[1fr_auto] divide-y divide-grid-dimmed lg:divide-x lg:divide-y-0 bg-background-2 py-6 rounded container">
        <div className="grid border-none col-span-12 lg:col-span-5">
          <div className="relative flex flex-col overflow-hidden">
            <div className="flex h-full max-w-lg flex-1 flex-col justify-center md:max-w-full">
              <h3 className="text-lg">AI Bug Enricher</h3>
              <p className="text-muted-foreground">
                Add context based tips on where to start, drop the barrier to
                resolve bugs
              </p>

              <div className="flex gap-2"></div>
            </div>
          </div>
        </div>

        <div className="col-span-2 border-none"></div>
        <div className="grid border-none col-span-12 lg:col-span-5">
          <Image
            src="/features/AI_BUG.png"
            width={1280}
            height={800}
            alt="AI Bug"
            className="dark:block rounded w-full"
          />
        </div>
      </div>

      <div className="relative grid border-none justify-center grid-cols-1 sm:grid-cols-12 divide-x-0 grid-rows-[1fr_auto] divide-y divide-grid-dimmed lg:divide-x lg:divide-y-0 bg-background-2 py-6 rounded container">
        <div className="grid border-none col-span-12 lg:col-span-5">
          <div className="relative flex flex-col overflow-hidden">
            <div className="flex h-full max-w-lg flex-1 flex-col justify-center md:max-w-full">
              <h3 className="text-lg">Seamless PR Tracking</h3>
              <p className="text-muted-foreground">
                Automatically create an issue to track a Pull Request (PR) as
                soon as it is assigned and linked to a Vantik issue
              </p>

              <div className="flex gap-2"></div>
            </div>
          </div>
        </div>
        <div className="col-span-2 border-none"></div>
        <div className="grid justify-center items-center border-none col-span-12 lg:col-span-5">
          <Image
            src="/features/PR.png"
            width={1280}
            height={800}
            alt="AI Bug"
            className="dark:block rounded w-full"
          />
        </div>
      </div>

      <div className="relative grid border-none justify-center grid-cols-1 sm:grid-cols-12 divide-x-0 grid-rows-[1fr_auto] divide-y divide-grid-dimmed lg:divide-x lg:divide-y-0 bg-background-2 py-6 rounded container">
        <div className="grid border-none col-span-12 lg:col-span-5">
          <div className="relative flex flex-col overflow-hidden">
            <div className="flex h-full max-w-lg flex-1 flex-col justify-center md:max-w-full">
              <h3 className="text-lg">Create Issues directly from Slack</h3>
              <p className="text-muted-foreground">
                Effortlessly create issues from Slack and keep your
                conversations in sync by using 👀 emoji
              </p>

              <div className="flex gap-2"></div>
            </div>
          </div>
        </div>
        <div className="col-span-2 border-none"></div>
        <div className="grid border-none col-span-12 lg:col-span-5">
          <Image
            src="/features/Slack.png"
            width={1280}
            height={800}
            alt="AI Bug"
            className="dark:block rounded w-full"
          />
        </div>
      </div>
    </div>
  );
}
