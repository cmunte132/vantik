---
title: "Self-hosting"
description: "You can self-host Vantik on your own infrastructure."
---
...
Should the burden ever get too much, we'd be happy to see you on [Vantik.ai cloud](https://app.vantik.dev) where we deal with these concerns for you.
...
3. Create a `.env` file. This file contains configuration for all the services used to run Vantik. Please fill in or modify the configurations as needed.
...
5. Run the start script and follow the prompts, We handle 2 major steps in start script setting up trigger.dev and setting up vantik for you.

```bash
./start.sh
```

6. You can now check vantik at `http://localhost:8000`
...
**Q: What is trigger.dev used for in Vantik?**
...
A: While the core functionality of Vantik will work, features like notifications and automated cycle transitions will not be available without trigger.dev properly configured.
